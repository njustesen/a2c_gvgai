...
def evaluate(policy, model_path, env_id, seed, nsteps=5, nstack=4, total_timesteps=1000, num_env=1, runs=100):
    if policy == 'cnn':
        policy_fn = CnnPolicy
    elif policy == 'lstm':
    ...
    model = Model(policy=policy_fn, ob_space=ob_space, ac_space=ac_space, nenvs=num_env, nsteps=nsteps, nstack=nstack, num_env=1)
...
def main():
    ...
    parser.add_argument('--model-path', help="Local path to the trained model")
    ...
    evaluate(args.policy, args.model_path, args.env, seed=1, num_env=args.num_envs, runs=args.runs)
