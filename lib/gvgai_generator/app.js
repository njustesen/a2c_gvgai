...
class WaitForBreakfast {
    ...
    getDifficultyParameters(diff) {
        let width = Math.floor(10 * diff + 3 * Math.random()) + 7;
        let height = Math.floor(10 * diff + 3 * Math.random()) + 7;
    ...
    generate(roomWidth, roomHeight, tableNumbers) {
        ...
        while (this.getDistance(start, waiter) < Math.max(roomWidth, roomHeight)) {
            waiter = {
                x: Math.round(Math.random()) * (roomWidth - 1),
                y: Math.floor(Math.random() * (roomHeight - 2)) + 1
            };
        ...
        while (this.getDistance(start, target) + this.getDistance(target, waiter) < Math.max(roomWidth, roomHeight) &&
            this.getDistance(start, target) < Math.max(roomWidth, roomHeight) / 2 &&
            this.getDistance(target, waiter) < Math.max(roomWidth, roomHeight) / 2) {
...
class Zelda {
    ...
    getDifficultyParameters(diff, maxWidth, maxHeight) {
        let width = Math.floor(diff * Math.max(maxWidth - 6, 0) + 2 * Math.random()) + 4;
        let height = Math.floor(diff * Math.max(maxHeight - 6, 0) + 2 * Math.random()) + 4;
        let openess = (1 - diff) * 0.4 + 0.1 * Math.random();
        let enemies = diff * 0.5 + 0.1 * Math.random() + 0.4;
        return [width, height, openess, enemies];
    }
    adjustParameters(width, height, openess, enemies) {
        let parameters = [openess, enemies];
    ...
    generate(width, height, openess, enemies) {
...
function padLevels(level, maxWidth, maxHeight, character) {
    ...
    //console.log(padLeft + " " + padRight + " " + padTop + " " + padBot);
    let result = "";
