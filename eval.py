...
def eval(policy, model_steps, env_id, seed, nsteps=5, nstack=1, total_timesteps=1000, num_env=1, runs=100, render=False):
    if policy == 'cnn':
        policy_fn = CnnPolicy
    elif policy == 'lstm':
...
def main():
    ...
    eval(args.policy, args.model_steps, args.env, seed=1, num_env=args.num_envs, runs=args.runs, render=args.render)
